...
Graph::Graph(){
    ...
    std::string headers[14] = {"openflightID", "name", "city", "country", "abbreviation", "ICAO", "latitude", "longitude", "altitude", "timezone", "DST", "timezone-database", "type", "source"};
    std::string route_headers[9] = {"airline_code", "airline_ID", "source_code", "source_ID", "destination_code", "destination_ID", "codeshare", "stops", "equipment"};
...
std::unordered_map<std::string, std::pair<Airport, std::vector<Route>>>& Graph::data(){
    return airports_;
}

std::vector<Route>& Graph::edge_data(){
    return routes_;
}