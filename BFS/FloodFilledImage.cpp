...
FloodFilledImage::~FloodFilledImage() { delete png_; }

/**
 * Adds a FloodFill operation to the FloodFillImage.  This function must store
...
Animation FloodFilledImage::animate(unsigned frameInterval) const {
    ...
    for (auto i = traversal_->begin(); i != traversal_->end(); ++i) {
        ...
        png_->getPixel(x, y) = colorPicker_->getColor(x, y);
        count++;
    }
    animation.addFrame(*png_);
